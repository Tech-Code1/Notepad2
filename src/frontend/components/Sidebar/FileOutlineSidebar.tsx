...
const FileOutlineSidebar: React.FC = () => {
  ...
  const isDirty = useFileStore(state => state.isDirty); // Selected isDirty
  
  const pagesForNotebook = useFileStore(state => 
    state.activeNotebookPath 
      ? state.getPagesForNotebook(state.activeNotebookPath).map(p => ({ 
          id: p.path, 
          name: p.path === state.currentFilePath && state.isDirty ? `${p.name} •` : p.name 
        }))
      : []
  );
  const loosePages = useFileStore(state => 
    state.getLoosePages().map(p => ({ 
      id: p.path, 
      name: p.path === state.currentFilePath && state.isDirty ? `${p.name} •` : p.name 
    }))
  );
  ...
  const createNewNotebook = useFileStore(state => state.createNewNotebook);
  ...
  const handleAddNewNotebook = async () => {
    if (!projectRootPath) {
      console.error("Cannot create notebook: Project root path is not set.");
      return;
    }
    try {
      const result = await createNewNotebook(projectRootPath); // Uses default name via prompt
      if (result.path) {
        console.log(`Notebook "${result.name}" created at ${result.path}`);
      } else {
        console.log("Notebook creation was cancelled or failed.");
      }
    } catch (error) {
    ...
    <aside className="w-72 min-w-[240px] bg-[#252526] text-gray-300 flex flex-col border-r border-gray-700">
      <div className="px-4 py-3 border-b border-gray-700 flex items-center justify-between">
        <h3 className="text-md font-medium text-white truncate flex-grow" title={currentFileNameForDisplay}>
          ...
          {isDirty && currentFilePath && <span className="text-yellow-500 ml-1" title="Cambios sin guardar">•</span>}
        </h3>
        <NavLink
